...
If the build is initiated by cApStAn, files in the `config` subfolder should be unlocked with `git-crypt unlock <key_file>` before running `gradle` to build the `jar`.
